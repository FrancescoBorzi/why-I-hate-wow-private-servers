...
Я помню, что в первый раз, когда я присоединился к одному из этих "особых миров", было много багов, но ты по крайней мере играл бесплатн. 
...
Я понятия не имел - я был так молод и неопытен в то время! Несмотря на это, я начал дурачится и, с помощью моего старого друга (Fabio), 
...
Это может прозвучать странно, но этот мир все еще очаровывает меня сейчас, как взрослого человека, в опредеенный момент я даже посвятил этой теме свою 
...
## Примечание переводчика: однако Blizzard разработывала вов не всей тысячей, всего над сервером и клентов трудилос человек 200 или около того.
...
## основателями MaNGOS, т.к. это по их инению затягивало исправления багов(что так и было, но проблему нужно не просто исправить,
## а сделать это качественно, например не добавив новых), но в итоге тринити пришли к тому же самому из-за чего ушли :D 
...
## Примечание переводчика: на самом деле CMaNGOS это тот же MaNGOS, только continued, парни не сошлись в взглядах на дальнейшую разработку
...
Таким образом, пиратские серверы достигли такого качества только благодаря большому комьюнити, 
...
Предположим, что у Кати и Вани есть частный сервер, и давайте предположим, что они находятся на одной и той же версии игры.
И Катя, и Ваня хотят выпустить новый контент для своих игроков, который до сих пор был закрыт, потому что боссы `A`, `B`, `C` и `D` довольно багованые.

- Катя очень хороший разработчик и может исправить обоих боссов `A` и `B`.
- Ваня все еще новичок и исправляет только босса `C`.

### В идеальном мире

Катя и Ваня работают вместе и обмениваются исправлениями. В результате, оба их сервера будут иметь отлично заскриптованых боссов `A`, `B` и `C`.
Кроме того, Катя и Ваня объединят свои усилия для работы над `D`.

В результате, игроки на обоих серверах очень довольны, потому что они получают полностью рабочий контент.

### В реальном мире

Катя и Ваня - соперники, поэтому они ведут войну друг против друга. На сервере Кати работают только боссы `А` и `Б`. В то время как на сервере Вани работает только `C`.
Некоторые игроки перемещаются с сервера Вани на сервер Кати. Сервер Вани закрывается через некоторое время. Некоторые игроки на сервере Кати перестают играть, потому что устают всегда ходить только на босов `А` и `В`, потому что `С` и `D` не работают.

В результате, игроки на обоих серверах менее довольны, чем в предыдущем сценарии. Катя заработала на донатах игроков больше, чем Ваня.
